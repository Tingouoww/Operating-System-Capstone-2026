...
static unsigned int page_refcount[BUDDY_MAX_PAGES];
...
static unsigned long pa_to_frame_idx(unsigned long pa)
{
    return addr_to_frame_idx(&buddy_allocator, pa);
}
...
void page_ref_inc(unsigned long pa)
{
    ...
    if (pa < buddy_allocator.base_addr ||
        pa >= frame_idx_to_addr(&buddy_allocator, buddy_allocator.page_count) ||
        (pa - buddy_allocator.base_addr) % PAGE_SIZE != 0)
    {
        return;
    }

    page_refcount[pa_to_frame_idx(pa)]++;
}
...
int page_ref_dec(unsigned long pa)
{
    ...
    idx = pa_to_frame_idx(pa);
    if (page_refcount[idx] > 0)
    {
        page_refcount[idx]--;
    }

    return (int)page_refcount[idx];
}
...
unsigned int page_ref_get(unsigned long pa)
{
    ...
    return page_refcount[pa_to_frame_idx(pa)];
}
...
void page_ref_set(unsigned long pa, unsigned int value)
{
    ...
    page_refcount[pa_to_frame_idx(pa)] = value;
}
