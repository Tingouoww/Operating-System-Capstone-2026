...
static unsigned long find_free_mmap_area(struct task_struct *t,
                                         unsigned long length,
                                         unsigned long hint) {
    unsigned long base = (hint >= USER_MMAP_BASE && hint < USER_MMAP_LIMIT)
                         ? hint : USER_MMAP_BASE;
    ...
    /* hint 之後沒有空間，從頭再找一次 hint 之前的區域 */
    if (hint > USER_MMAP_BASE) {
        base = USER_MMAP_BASE;
        while (base + length <= hint) {
            if (range_is_available(t, base, length))
                return base;
            base += PAGE_SIZE;
        }
...
static struct vma *create_vma(struct task_struct *t,
    ...
    v->used = 1;
    v->type = type;
    v->src = src;  // 資料來源（VMA_TEXT 用：kernel VA of binary）
    v->src_len = src_len; // 資料來源長度
    return v;
}
...
long sys_mmap(unsigned long addr, unsigned long length, int prot, int flags) {
    ...
    // 對齊且不重疊就用這個 addr 當作 start
    if (addr != 0 && is_page_aligned(addr) &&
        range_is_available(cur, addr, length)) {
        start = addr;
    } else {
        // addr 不符合條件時，把它 page-align 後當作 hint 去附近找
        unsigned long hint = addr ? page_align_up(addr) : 0;
        start = find_free_mmap_area(cur, length, hint);
        if (start == 0)
            return 0;
    }
    ...
    // MAP_POPULATE = 立刻分配
    if ((flags & MAP_POPULATE) && (vma_pte_flags(v) & (PTE_R | PTE_W | PTE_X))) {
...
/* 組出實際對應的 page table entry flags */
...
int setup_user_exec_vmas(struct task_struct *t,
    ...
    if (!create_vma(t, USER_CODE_VA, code_end,
        ...
        return -1;
    // VMA_STACK 使用者stack, 放 function call、local variable
    if (!create_vma(t, stack_start, USER_STACK_VA + PAGE_SIZE,
                    PROT_READ | PROT_WRITE, MAP_ANONYMOUS,
                    VMA_STACK, 0, 0))
        return -1;
    // VMA_SIGNAL_STACK kernel 要跳進 signal handler 前會先把 context 存到這個 stack 上面(不破壞原本的 user stack)
    if (!create_vma(t, USER_SIGNAL_STACK_VA,
                    USER_SIGNAL_STACK_VA + PAGE_SIZE,
                    PROT_READ | PROT_WRITE, MAP_ANONYMOUS,
                    VMA_SIGNAL_STACK, 0, 0))
        return -1;
    // VMA_TRAMPOLINE signal return 跳板
...
/* 為某個 VMA 區域內的特定位址實際分配並建立一個 physical page 的映射 */
